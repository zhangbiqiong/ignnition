...
def train_and_evaluate(model):
    ...
    eval_spec = tf.estimator.EvalSpec(
        input_fn=lambda: input_fn(filenames_eval,
                                  shuffle=str_to_bool(CONFIG['TRAINING_OPTIONS']['shuffle_eval_samples'])),
        throttle_secs=int(CONFIG['TRAINING_OPTIONS']['throttle_secs']),
        steps=int(CONFIG['TRAINING_OPTIONS']['eval_samples']))
...
def debug(model_description):
    ...
    print()
    tf.compat.v1.logging.warn(
        'IGNNITION: Generating the debug model... \n---------------------------------------------------------\n')
